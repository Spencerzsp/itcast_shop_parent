...
//        new Main().run();
    ...
    private static void runNgnix() throws Exception {
        String format = "%h %u %l %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\"";
        String logline = "47.103.150.125 - - [14/Dec/2020:16:22:03 +0800] \"GET /admin/ HTTP/1.1\" 404 570 \"-\" \"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Safari/537.36\"";
//        printAllPossibles(format);

        HttpdLoglineParser<NgnixRecord> parser = new HttpdLoglineParser<>(NgnixRecord.class, format);
        NgnixRecord ngnixRecord = new NgnixRecord();
        parser.parse(ngnixRecord, logline);
//        System.out.println(ngnixRecord);
        System.out.println(ngnixRecord);

//        System.out.println(ngnixRecord.toString());
